...
public class SeguimientoEstudianteActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_seguimiento_estudiante);
    ...
    private void cargarSeguimientos() {
        ...
        service.getSeguimientosEstudiante().enqueue(new Callback<SeguimientoEstudianteResponse>() {
            @Override
            public void onResponse(Call<SeguimientoEstudianteResponse> call, Response<SeguimientoEstudianteResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (SeguimientoEstudianteResponse.Seguimiento seg : lista) {
                        View card = crearCard(seg);
                        contenedor.addView(card);
                    }

                    if (lista.isEmpty()) {
                        Toast.makeText(SeguimientoEstudianteActivity.this, "Sin seguimientos disponibles.", Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(SeguimientoEstudianteActivity.this, "Error al obtener datos", Toast.LENGTH_SHORT).show();
                }
            }

            @Override
            public void onFailure(Call<SeguimientoEstudianteResponse> call, Throwable t) {
                Toast.makeText(SeguimientoEstudianteActivity.this, "Error de conexión", Toast.LENGTH_SHORT).show();
            }
        });
    }
