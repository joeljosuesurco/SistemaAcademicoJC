...
const menuClick = async (event) => {
  ...
  if (props.item?.label === 'Comunicados') {
...
<template>
  ...
  >
    ...
    >
      ...
      <!-- Ícono con badge si es Comunicados -->
      <div class="relative">
        <BaseIcon v-if="item?.icon" :path="item.icon" class="transition-colors" />
        <span
          v-if="item?.label === 'Comunicados' && mainStore.comunicadosVisibles.length > 0"
          class="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] px-1.5 py-0.5 rounded-full font-bold"
        >
          {{ mainStore.comunicadosVisibles.length }}
        </span>
      </div>
