  ...
  <div class="p-6">
    ...
    <div v-if="estudianteSeleccionado" class="bg-white border rounded shadow p-6">
      ...
      <div v-if="!mostrarBoleta" class="overflow-x-auto">
        <table class="min-w-full text-center border border-gray-200 bg-white rounded-xl shadow-sm">
          <thead>
            <tr>
          ...
          <tbody>
            ...
            >
              ...
              <td class="border px-6 py-3">
                {{ m.decidir_raw }} <span class="text-xs text-gray-500">({{ m.decidir_pct }})</span>
              </td>
              <td class="border px-6 py-3">
                {{ m.ser_auto_raw }}
    ...
    <div v-else>
      ...
      <div class="mt-4 flex justify-center items-center gap-2" v-if="totalPages > 1">
        <BaseButton label="« Anterior" small :disabled="currentPage === 1" @click="currentPage--" />
