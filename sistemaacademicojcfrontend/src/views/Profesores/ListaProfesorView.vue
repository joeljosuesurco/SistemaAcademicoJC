...
async function verMaterias(idProfesor) {
  ...
  try {
    ...
    const profesor = res.data.data

    profesorSeleccionado.value = {
      ...profesor,
      apellidos_pat: profesor.persona_rol?.persona?.apellidos_pat || '',
      apellidos_mat: profesor.persona_rol?.persona?.apellidos_mat || '',
      nombres: profesor.persona_rol?.persona?.nombres_persona || '',
      fecha_nacimiento: profesor.persona_rol?.persona?.fecha_nacimiento || '',
      celular: profesor.persona_rol?.persona?.celular_persona || '',
      direccion: profesor.persona_rol?.persona?.direccion_persona || '',
      nacionalidad: profesor.persona_rol?.persona?.nacionalidad_persona || '',
      sexo: profesor.persona_rol?.persona?.sexo_persona || '',
      fotografia: profesor.persona_rol?.persona?.fotografia_persona || '',
      ci: profesor.persona_rol?.persona?.documento?.carnet_identidad || '',
      certificado_nacimiento:
        profesor.persona_rol?.persona?.documento?.certificado_nacimiento || '',
    }

    materiasAsignadas.value = profesor.curso_profesor_materia_gestion || []
  } catch (error) {
      ...
      <div v-if="mostrarDetalle && profesorSeleccionado" ref="detalleRef" class="mt-6">
          ...
          <div class="border border-blue-300 p-6 rounded shadow-xl bg-white">
            ...
            <div class="flex gap-6 items-start">
              ...
              <div class="space-y-1">
                ...
                <p>
                  <strong>Especialidad:</strong> {{ profesorSeleccionado.especialidad_profesor }}
                </p>
        ...
        >
          ...
          <table v-else class="w-full text-sm border">
            ...
            <tbody>
              <tr v-for="asignacion in materiasAsignadas" :key="asignacion.id">
                <td class="p-2">{{ asignacion.materia?.campo_materia || '—' }}</td>
                <td class="p-2">{{ asignacion.materia?.sigla_materia || '—' }}</td>
                <td class="p-2">{{ asignacion.materia?.nivel_educativo?.nombre || '—' }}</td>
                <td class="p-2">
                  {{ asignacion.curso?.grado_curso || '' }}
                  {{ asignacion.curso?.paralelo_curso || '' }}
                  - {{ asignacion.curso?.nivel_educativo?.nombre || '' }}
                </td>
                <td class="p-2">{{ asignacion.gestion?.nombre_gestion || '—' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
