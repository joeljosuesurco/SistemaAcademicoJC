...
const padresFiltrados = computed(() => {
  ...
  const lista = padres.value.filter((p) => {
    const nombreCompleto = `${p.apellidos_pat} ${p.apellidos_mat} ${p.nombres}`.toLowerCase()
    return nombreCompleto.includes(filtro) || p.ci.toLowerCase().includes(filtro)
  })
...
const totalPages = computed(() => {
  const total = search.value
    ? padres.value.filter((p) => {
        const nombreCompleto = `${p.apellidos_pat} ${p.apellidos_mat} ${p.nombres}`.toLowerCase()
        return (
          nombreCompleto.includes(search.value.toLowerCase()) ||
          p.ci.toLowerCase().includes(search.value.toLowerCase())
        )
      }).length
    : padres.value.length
        ...
        <div class="border border-blue-300 p-6 rounded shadow-xl bg-white">
          ...
          <div class="flex gap-6 items-start">
            ...
            <div class="space-y-1">
              ...
              <p><strong>Estado:</strong> {{ padreSeleccionado.estado_padre }}</p>
            </div>
          </div>
        </div>
          ...
          <ul class="list-disc list-inside text-sm space-y-2">
            ...
            <li>
              ...
              <span
                class="ml-2"
                :class="
                  padreSeleccionado.certificado_nacimiento ? 'text-green-600' : 'text-red-600'
                "
              >
